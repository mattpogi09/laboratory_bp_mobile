...
import { Edit, Layers, Plus, Power, PowerOff, Trash2, X } from "lucide-react-native";
import { useCallback, useState } from "react";

import api from "@/app/services/api";
import { showApiError } from "@/app/utils";
import { ConfirmDialog, SuccessDialog } from "@/components";
import type { InventoryCategory } from "@/app/types";
...
export default function InventoryCategoriesScreen() {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            setSuccessDialog({ visible: true, title: "Created", message: "Inventory category created.", type: "success" });
            loadCategories();
        } catch (err: any) {
            showApiError(err);
        }
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            setSelected(null);
            setSuccessDialog({ visible: true, title: "Updated", message: "Inventory category updated.", type: "success" });
            loadCategories();
        } catch (err: any) {
            showApiError(err);
        }
    ...
    const handleToggle = (cat: InventoryCategory) => {
        setConfirmDialog({
            visible: true,
            ...
            onConfirm: async () => {
                ...
                try {
                    await api.post(`/inventory-categories/${cat.id}/toggle`);
                    setSuccessDialog({ visible: true, title: "Done", message: "Status updated.", type: "success" });
                    loadCategories();
                } catch (err: any) {
                    showApiError(err);
                }
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    setSuccessDialog({ visible: true, title: "Deleted", message: "Category deleted.", type: "success" });
                    loadCategories();
                } catch (err: any) {
                    showApiError(err);
                }
        ...
        <View style={styles.card}>
            ...
            <View style={styles.cardActions}>
                ...
                <TouchableOpacity style={styles.iconBtn} onPress={() => handleToggle(item)}>
                    {item.is_active ? <PowerOff size={16} color="#EF4444" /> : <Power size={16} color="#22C55E" />}
                </TouchableOpacity>
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <View style={styles.headerLeft}>
                    ...
                    <Text style={styles.headerTitle}>Inventory Categories</Text>
                </View>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.empty}>
                            ...
                            <Text style={styles.emptyText}>No inventory categories yet</Text>
                        </View>
                    }
                />
            )}
            ...
            {[
                { visible: showCreateModal, onClose: () => setShowCreateModal(false), onSave: handleCreate, title: "New Inventory Category" },
                { visible: showEditModal, onClose: () => setShowEditModal(false), onSave: handleUpdate, title: "Edit Inventory Category" },
...
const styles = StyleSheet.create({
    ...
    addBtn: {
        ...
        backgroundColor: "#ac3434", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8,
    },
